...
class NotificationController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        // echo "<pre>"; print_r(count($user_list));die();
        ...
        return view('messages.index', compact('user', 'user_list', 'show', 'id'))->with($data);
    }
...
public function sendto_all(Request $request){

   // echo $request->input('messagetoall');
   

        $response = array();
        $response['code'] = 400;
        $user = Auth::user();
        $friendlist = User::all()->where('id','!=',$user->id);
        //$friend = User::find($request->input('id'));
//echo "<pre>"; print_r($friendlist);
        ...
        foreach($friendlist as $friend)
        {
            if ($friend){
                ...
                $message->sender_user_id = $user->id;
                $message->receiver_user_id = $friend->id;
                $message->message = $request->input('messagetoall');
                if ($message->save()){
                    $response['code'] = 200;
                    $html = View::make('messages.widgets.single_message', compact('user', 'message'));
                    $response['html'] = $html->render();
                    $response['message_id'] = $message->id;
                    $request->session()->flash('status', 'Message send successfully to all.');
                }
            }
        }
