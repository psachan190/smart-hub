...
function deletePost(id){
    ...
    BootstrapDialog.show({
        ...
        buttons: [{
            label: "Yes, I'm Sure!",
            cssClass: 'btn btn-sm btn--round btn-warning',
...
function submitComment(id){
    ...
    var data = new FormData();
    data.append('id', id);
    var comment = $('#panel-post-'+id+' #form-new-comment input').val();
    ...
    if (comment.trim() == ''){
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.code == 200) {
                    $('#panel-post-'+id+' #form-new-comment input').val("");
