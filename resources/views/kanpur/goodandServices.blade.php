					...
					<div class="filter__option filter--dropdown">
						...
						<ul class="dropdown dropdown-menu" aria-labelledby="drop1">
							<?php
								...
								?>
							<li><a href="<?php echo url("goods_services")."/".base64_encode($shopDetails->username)."/".base64_encode($list->cid); ?>"><?php if(!empty($list->cname)) echo $list->cname; ?></a></li>
							<?php
								}
								 }
								 ?> 
		...
		<div class="row">
			<div class="col-sm-12 col-md-3 padding_div">
				...
				<div style="margin-top:20px;">
					<ul class="mdn-accordion">
						...
						$shopName = str_replace(" ","_",$shopDetails->vName);
						$serviceID = base64_encode($list->cid);
						@endphp
						<li><a href="<?php echo url("services_shop/$shopID/$serviceID"); ?>"><i class="fa fa-angle-double-right"></i>{{ $list->cname }}</a></li>
						@endforeach
						@endif
					</ul>
				</div>
			...
			<div class="col-sm-12 col-md-9">
				<div class="row">
					...
					<div class="col-md-12">
						<div class="testimonial-slider testislider">
							<?php
								foreach($offerNewsList as $listArr){
								$offerID = base64_encode($listArr->id);
								$title = str_replace(" ","_",$listArr->newsTitle);
								$image = $listArr->image;
								if(!empty($listArr->discountMode)){
								?>
							<div class="testimonial">
								<div class="testimonial__about">
									<a href="<?php echo url("kanpurize_offers_products/$offerID/$title"); ?>"> 
									<img src='{{ url("uploadFiles/offersNews/$image") }}' alt='{{ $title }}' />
									</a >
								</div>
							</div>
							<?php
