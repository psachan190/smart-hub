			...
			<tr>
             <input type="hidden" name="orderID" class="orderID" id="orderID" value="@if(!empty($list->id)){{ $list->id }}@endif" />
              <td>@if(!empty($list->orderDate)){{ $list->orderDate }} @endif</td>
               <td>@if(!empty($list->id)){{ "ORD-".$list->id }} @endif</td>
              ...
              <td>
                <a href='{{ url("vendor_orderDetails/$id") }}' class="btn btn-success">Order Details</a>&nbsp;<a href='#' class="btn btn-warning dispatch" id="dispatch" >Dispatch</a>
             </td>
           </tr>
				  @endforeach
			   @else
...
$(document).ready(function(e) {
 /*--cancel order script start--*/ 
   $(document).on('click','.dispatch',function(){
	 $("#orderResult").html(" "); 
	 var con = confirm("Are you sure want to dispatch  this order ."); 
