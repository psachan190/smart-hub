          ...
          <tbody>
               @if($productList != FALSE)
				  @foreach($productList as $list)
					 ...
					 @endphp
			<tr>
             <td><input type="" name="prdID" id="prdID" class="prdID" value="{{ $list->id }}" /></td>
              ...
              <td colspan="3">
               <a href='{{ url("Kanpurize_Vendor_productsDetails?productsid=$id&kanpurizeShop=$shopID") }}'  class="btn btn-sm btn--round btn-success">Products Details</a>
                              <button class="btn btn-sm btn--round btn-danger trashProducts" id="{{ $list->id }}">Restore</button>
                               <button class="btn btn-sm btn--round btn-danger deletePrdBtn" id="deletePrdBtn">Delete</button>
                            </td>
           </tr>
				  @endforeach
			   @else
  ...
  $(document).on('click',".trashProducts",(function(e) {
    var con = confirm("Are  you sure want to products goes on trash folder.");
     if(con == true){ 
		...
		$.ajax({
			type:'GET',
			url:'<?php echo url("restoreTrashProducts"); ?>',
...
<script>
 /*products permanent delete script start--*/
$(document).ready(function(e) {
   $(document).on('click','.deletePrdBtn',function(){
     var con = confirm("Are  you sure want to Permanent delete this products .");
     if(con == true){
		$(this).html("please Wait..").attr('disabled',true); 
        var productsID = $(this).parent().parent().find('.prdID').val();
		$.ajax({
			type:'GET',
			url:'<?php echo url("permanentDeleteProducts"); ?>',
...
/*products permanent delete script End--*/
</script>
